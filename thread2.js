...
async function main() {
    ...
    MongoClient.connect(DBurl, { useNewUrlParser: true }, async (err, client) => {
        ...
        let faulty = []
        for (let i = cityList.length/2; i < cityList.length; i++) {
...
main()
let baseUrl = 'https://www.hospitalityonline.com/marriott/locations'

//process_child(baseUrl)
    ...
    if (response.status === 200) {
        ...
        while (!end) {
            ...
            if (body.length < 10) {
                ...
                let collection2 = db.collection("childdetails8")
